...
def main():
    target_dir = Path(os.path.dirname(__file__) + '\\spacy_models\\final_model')
    target_dir.mkdir(exist_ok=True)

    pipeline = []

    source_dir = Path(os.path.dirname(__file__) + '\\spacy_models\\dependency_model\\model-best')
    ...
    source_dir = Path(os.path.dirname(__file__) + '\\spacy_models\\ner_model\\model-best')
